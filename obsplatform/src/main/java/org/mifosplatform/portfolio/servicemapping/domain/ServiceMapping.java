...
public class ServiceMapping extends AbstractPersistable<Long> {
	...
	public ServiceMapping(final Long serviceId, final String serviceUrl,
			final String status, final String image, final String category,
	...
	public static ServiceMapping fromJson(JsonCommand command) {
		...
		final String serviceIdentification = command
				.stringValueOfParameterNamed("serviceIdentification");
		...
		final String subcategory = command
				.stringValueOfParameterNamed("subCategory");

		return new ServiceMapping(serviceId, serviceIdentification, status,
	...
	public Map<String, Object> update(JsonCommand command) {
		...
		if (command.isChangeInLongParameterNamed(serviceParamName,
				this.serviceId)) {
		...
		if (command.isChangeInStringParameterNamed(
				serviceIdentificationParamName, this.serviceIdentification)) {
			final String newValue = command
					.stringValueOfParameterNamed(serviceIdentificationParamName);
			actualChanges.put(serviceIdentificationParamName, newValue);
			this.serviceIdentification = StringUtils.defaultIfEmpty(newValue,
					null);
		}

		final String statusParamName = "status";
		if (command
				.isChangeInStringParameterNamed(statusParamName, this.status)) {
			final String newValue = command
		...
		if (command.isChangeInStringParameterNamed(imageParamName, this.image)) {
			final String newValue = command
					.stringValueOfParameterNamed(imageParamName);
		...
		if (command.isChangeInStringParameterNamed(categoryParamName,
				this.category)) {
			final String newValue = command
					.stringValueOfParameterNamed(categoryParamName);
		...
		if (command.isChangeInStringParameterNamed(subCategoryParamName,
				this.subCategory)) {
			final String newValue = command
					.stringValueOfParameterNamed(subCategoryParamName);
