...
public class ProcessEventActionServiceImpl implements ProcessEventActionService {
	...
	@Autowired
	public ProcessEventActionServiceImpl(
	...
	@Override
	@Transactional
	public void processEventActions(final EventActionData eventActionData) {
		...
		try {
			...
			if (eventActionData.getActionName().equalsIgnoreCase(EventActionConstants.ACTION_RENEWAL)) {
				...
				final String jsonObject = eventActionData.getJsonData();
				final JsonElement parsedCommand = this.fromApiJsonHelper.parse(jsonObject);
				final JsonCommand command = JsonCommand.from(jsonObject, parsedCommand, 
						this.fromApiJsonHelper, "RenewalOrder", eventActionData.getClientId(),
						null, null, eventActionData.getClientId(), null, 
						null, null, null, null, null, null);

				this.orderWritePlatformService.renewalClientOrder(command, eventActionData.getOrderId());
				/**
				 * OrderHistory orderHistory=new
				 * OrderHistory(eventActionData.getOrderId(),new LocalDate(),new
				 * LocalDate(),null,"Renewal",Long.valueOf(0),null); Order
				 * order=
				 * this.orderRepository.findOne(eventActionData.getOrderId());
				 * this.orderHistory.save(orderHistory);
				 * 
				 * transactionHistoryWritePlatformService.saveTransactionHistory(
				 * eventActionData
				 * .getClientId(),"ORDER_"+UserActionStatusTypeEnum
				 * .RENEWAL_BEFORE_AUTOEXIPIRY.toString(), order.getStartDate(),
				 * "PlanId:"
				 * +order.getPlanId(),"Renewal Period: 1 Month","OrderID:"
				 * +order.getId(),"Billing Align:"+order.getbillAlign());
				 */
			} else if (eventActionData.getActionName().equalsIgnoreCase(EventActionConstants.ACTION_ACTIVE)) {

				// Order
				// order=this.orderRepository.findOne(eventActionData.getOrderId());
				this.orderWritePlatformService.reconnectOrder(eventActionData.getOrderId());
				/*
				 * transactionHistoryWritePlatformService.saveTransactionHistory(
				 * eventActionData
				 * .getClientId(),"ORDER_"+UserActionStatusTypeEnum
				 * .RECONNECTION.toString(), order.getStartDate(),
				 * "Price:"+order
				 * .getAllPriceAsString(),"PlanId:"+order.getPlanId
				 * (),"contarctPeriod: 1 Month"
				 * ,"Services:"+order.getAllServicesAsString
				 * (),"OrderID:"+order.getId
				 * (),"Billing Align:"+order.getbillAlign());
				 */
				...
				final String jsonObject = eventActionData.getJsonData();
				final JsonElement parsedCommand = this.fromApiJsonHelper.parse(jsonObject);
				final JsonCommand command = JsonCommand.from(jsonObject,
						parsedCommand, this.fromApiJsonHelper,
						"DissconnectOrder", eventActionData.getClientId(),
						null, null, eventActionData.getClientId(), null, null,
						null, null, null, null, null);
				this.orderWritePlatformService.disconnectOrder(command,
						eventActionData.getOrderId());
				...
				final JsonElement parsedCommand = this.fromApiJsonHelper.parse(jsonObject);
				final JsonCommand command = JsonCommand.from(jsonObject,
						parsedCommand, this.fromApiJsonHelper, "CreateOrder",
						eventActionData.getClientId(), null, null,
						eventActionData.getClientId(), null, null, null, null,
						null, null, null);

				CommandProcessingResult commandProcessingResult = this.orderWritePlatformService
						.createOrder(eventActionData.getClientId(), command);
				/*
				 * //For Transaction History
				 * transactionHistoryWritePlatformService
				 * .saveTransactionHistory(eventActionData.getClientId(),
				 * "New Order", new Date(),
				 * "PlanId:"+plancode,"contarctPeriod: One Month"
				 * ,"OrderID:"+commandProcessingResult.resourceId(),
				 * "BillingAlign:false");
				 */
				...
				try {
					...
					final JsonElement parsedCommand = this.fromApiJsonHelper.parse(jsonObject);
					final JsonCommand command = JsonCommand.from(jsonObject, parsedCommand, 
							this.fromApiJsonHelper, "CreateInvoice", eventActionData.getClientId(),
							null, null, eventActionData.getClientId(), null,
							null, null, null, null, null, null);

					// CommandProcessingResult
					// commandProcessingResult=this.orderWritePlatformService.createOrder(eventActionData.getClientId(),
					// command);
				...
				try {
					...
					final List<HardwareAssociationData> associationDatas = this.hardwareAssociationReadplatformService
							.retrieveClientAllocatedHardwareDetails(eventActionData.getClientId());
					
					if (!associationDatas.isEmpty()) {
						Long none = Long.valueOf(0);
						ProcessRequest processRequest = new ProcessRequest(none, eventActionData.getClientId(), none,
								ProvisioningApiConstants.PROV_BEENIUS, ProvisioningApiConstants.REQUEST_TERMINATE,
								'N', 'N');
						ProcessRequestDetails processRequestDetails = new ProcessRequestDetails(
								none, none, null, "success", associationDatas.get(0).getProvSerialNum(), new Date(),
								null, new Date(), null, 'N', ProvisioningApiConstants.REQUEST_TERMINATE, null);
			...
			// EventAction
			// eventAction=this.eventActionRepository.findOne(eventActionData.getId());
			eventAction.updateStatus('F');
			this.eventActionRepository.save(eventAction);
